...
import { saveTradeCore, rollbackCore } from "@/app/(app)/trades/[slug]/core";
...
describe("trade editor actions", () => {
  it("save creates+activates a new version; rollback re-activates an older one", async () => {
    ...
    await saveTradeCore({ slug, nameAr: "أعمال البلاط", content: { ...content("15"), trade: slug }, changelog: "أول" }, db);
    const v1 = await getActiveSkill(slug);
    expect(v1?.content.costModels[0].markupPct).toBe("15");
    expect(v1?.versionNumber).toBe(1);

    await saveTradeCore({ slug, nameAr: "أعمال البلاط", content: { ...content("20"), trade: slug }, changelog: "رفع الربح" }, db);
    ...
    await rollbackCore(slug, v1id, db);
    expect((await getActiveSkill(slug))?.versionNumber).toBe(1);
  });

  it("rejects invalid content via the schema", async () => {
    await expect(saveTradeCore({ slug: `bad-${Date.now()}`, nameAr: "سيئ", content: { trade: "x", costModels: [{ nope: true }] } as never, changelog: "x" }, db)).rejects.toThrow();
  });
});
